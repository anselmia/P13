...
! function (a, b, c) {
    ...
    function B(a, b, c) {
        ...
        if (g = b.drag, e = g.layer, h = e && e.dragGroups || [], d = b.layers, "mousemove" === c || "touchmove" === c) {
            if (g.dragging || (g.dragging = !0, e.dragging = !0, e.bringToFront && (d.splice(e.index, 1), e.index = d.push(e)), e._startX = e.x, e._startY = e.y, e._endX = e._eventX, e._endY = e._eventY, G(a, b, e, "dragstart")), g.dragging)
                for (l = e._eventX - (e._endX - e._startX), m = e._eventY - (e._endY - e._startY), e.updateDragX && (l = e.updateDragX.call(a[0], e, l)), e.updateDragY && (m = e.updateDragY.call(a[0], e, m)), e.dx = l - e.x, e.dy = m - e.y, "y" !== e.restrictDragToAxis && (e.x = l), "x" !== e.restrictDragToAxis && (e.y = m), G(a, b, e, "drag"), k = 0; k < h.length; k += 1)
    ...
    function N(a) {
        var b, c = 3;
        for ("array" !== ta(a.start) && (a.start = M(a.start), a.end = M(a.end)), a.now = [], 1 === a.start[3] && 1 === a.end[3] || (c = 4), b = 0; b < c; b += 1) a.now[b] = a.start[b] + (a.end[b] - a.start[b]) * a.pos, b < 3 && (a.now[b] = wa(a.now[b]));
    ...
    d.baseDefaults = {
            ...
            var b, c, d, e, g, h = this,
                i = [];
            if (0 !== h.length)
                if (b = h[0], c = s(b), d = c.layers, f(a))
                    for (g = 0; g < d.length; g += 1) e = d[g], a.call(b, e) && i.push(e);
                else i = d;
            return i
            ...
            for (e = 0; e < m.length; e += 1)
                if (d = a(m[e]), f = s(m[e]), h = a(m[e]).getLayer(b)) {
                    ...
                    for (j in c) Object.prototype.hasOwnProperty.call(c, j) && (k = c[j], l = ta(k), "object" === l && ua(k) ? (h[j] = ra({}, k), i(h[j])) : "array" === l ? h[j] = k.slice(0) : "string" === l ? 0 === k.indexOf("+=") ? h[j] += qa(k.substr(2)) : 0 === k.indexOf("-=") ? h[j] -= qa(k.substr(2)) : !isNaN(k) && g(k) && "text" !== j ? h[j] = qa(k) : h[j] = k : h[j] = k);
            ...
            for (e = 0; e < i.length; e += 1) d = a(i[e]), f = s(i[e]), g = f.layers, (h = d.getLayer(b)) && (h.index = sa(h, g), g.splice(h.index, 1), g.splice(c, 0, h), c < 0 && (c = g.length + c), h.index = c, G(d, f, h, "move"));
            return i
        }, a.fn.removeLayer = function (b) {
            var c, d, e, f, g, h = this;
            for (d = 0; d < h.length; d += 1) c = a(h[d]), e = s(h[d]), f = c.getLayers(), (g = c.getLayer(b)) && (g.index = sa(g, f), f.splice(g.index, 1), delete g._layer, x(c, e, g, {
                name: null
            }), y(c, e, g, {
                groups: null
            }), G(c, e, g, "remove"));
            ...
            var d, e, f, g = this,
                h = [c];
            for (e = 0; e < g.length; e += 1) d = a(g[e]), f = d.getLayer(b), f.groups && (h = f.groups.slice(0), -1 === sa(c, f.groups) && h.push(c)), d.setLayer(f, {
                groups: h
            });
            return g
            ...
            var d, e, f, g, h = this,
                i = [];
            for (e = 0; e < h.length; e += 1) d = a(h[e]), f = d.getLayer(b), f.groups && -1 !== (g = sa(c, f.groups)) && (i = f.groups.slice(0), i.splice(g, 1), d.setLayer(f, {
                groups: i
            }));
            return h
            ...
            var c, d, e, f, g, i, k, l, m, n, o, p, q, r = this,
                t = b || {};
        ...
        function (a) {
            ...
            if (Da.drawings[g.type] && "function" !== g.type) f[Da.drawings[g.type]](b);
                ...
                for (d = 0; d < f.length; d += 1)(e = h(f[d])) && (g = new c(b), H(f[d], g, b, a), g.visible && g.fn && g.fn.call(f[d], e, g));
            return f
        }, a.fn.clearCanvas = function a(b) {
            var d, e, f = this,
                g = new c(b);
            for (d = 0; d < f.length; d += 1)(e = h(f[d])) && (null === g.width || null === g.height ? (e.save(), e.setTransform(1, 0, 0, 1, 0, 0), e.clearRect(0, 0, f[d].width, f[d].height), e.restore()) : (H(f[d], g, b, a), r(f[d], e, g, g.width, g.height), e.clearRect(g.x - g.width / 2, g.y - g.height / 2, g.width, g.height), p(e, g)));
            return f
        }, a.fn.saveCanvas = function a(b) {
            var d, e, f, g, i, j = this;
            for (d = 0; d < j.length; d += 1)
                if (e = h(j[d]))
                    for (g = s(j[d]), f = new c(b), H(j[d], f, b, a), i = 0; i < f.count; i += 1) k(e, g);
            return j
        }, a.fn.restoreCanvas = function a(b) {
            var d, e, f, g, i, j = this;
            for (d = 0; d < j.length; d += 1)
                if (e = h(j[d]))
                    for (g = s(j[d]), f = new c(b), H(j[d], f, b, a), i = 0; i < f.count; i += 1) l(e, g);
            return j
            ...
            var d, e, f, g, i, j = this;
            for (d = 0; d < j.length; d += 1)(e = h(j[d])) && (f = new c(b), H(j[d], f, b, a), f.visible && (r(j[d], e, f, f.width, f.height), n(j[d], e, f), g = f.width * (4 / 3), i = f.height, e.beginPath(), e.moveTo(f.x, f.y - i / 2), e.bezierCurveTo(f.x - g / 2, f.y - i / 2, f.x - g / 2, f.y + i / 2, f.x, f.y + i / 2), e.bezierCurveTo(f.x + g / 2, f.y + i / 2, f.x + g / 2, f.y - i / 2, f.x, f.y - i / 2), R(j[d], e, f), f.closed = !0, q(j[d], e, f)));
            return j
        }, a.fn.drawPolygon = function a(b) {
            var d, e, f, g, i, j, k, l, m, o, p = this;
            for (d = 0; d < p.length; d += 1)
                if ((e = h(p[d])) && (f = new c(b), H(p[d], f, b, a), f.visible)) {
                    for (r(p[d], e, f, 2 * f.radius), n(p[d], e, f), i = 2 * va / f.sides, j = i / 2, g = j + va / 2, k = f.radius * za(j), e.beginPath(), o = 0; o < f.sides; o += 1) l = f.x + f.radius * za(g), m = f.y + f.radius * ya(g), e.lineTo(l, m), f.concavity && (l = f.x + (k + -k * f.concavity) * za(g + j), m = f.y + (k + -k * f.concavity) * ya(g + j), e.lineTo(l, m)), g += i;
                    R(p[d], e, f), f.closed = !0, q(p[d], e, f)
                } return p
        }, a.fn.drawSlice = function a(b) {
            var d, e, f, g, i, j, k = this;
            for (d = 0; d < k.length; d += 1)(e = h(k[d])) && (f = new c(b), H(k[d], f, b, a), f.visible && (r(k[d], e, f, 2 * f.radius), n(k[d], e, f), f.start *= f._toRad, f.end *= f._toRad, f.start -= va / 2, f.end -= va / 2, f.start = V(f.start), f.end = V(f.end), f.end < f.start && (f.end += 2 * va), g = (f.start + f.end) / 2, i = f.radius * f.spread * za(g), j = f.radius * f.spread * ya(g), f.x += i, f.y += j, e.beginPath(), e.arc(f.x, f.y, f.radius, f.start, f.end, f.ccw), e.lineTo(f.x, f.y), R(k[d], e, f), f.closed = !0, q(k[d], e, f)));
            return k
            ...
            for (d = 0; d < v.length; d += 1)
                if ((e = h(v[d])) && (f = new c(b), H(v[d], f, b, a), f.visible)) {
                    if (e.textBaseline = f.baseline, e.textAlign = f.align, ga(v[d], e, f), i = null !== f.maxWidth ? ia(e, f) : f.text.toString().split("\n"), ha(v[d], e, f, i), g && (g.width = f.width, g.height = f.height), r(v[d], e, f, f.width, f.height), n(v[d], e, f), t = f.x, "left" === f.align ? f.respectAlign ? f.x += f.width / 2 : t -= f.width / 2 : "right" === f.align && (f.respectAlign ? f.x -= f.width / 2 : t += f.width / 2), f.radius)
                        for (l = qa(f.fontSize), null === f.letterSpacing && (f.letterSpacing = l / 500), k = 0; k < i.length; k += 1) {
                            for (e.save(), e.translate(f.x, f.y), j = i[k], f.flipArcText && (o = j.split(""), o.reverse(), j = o.join("")), m = j.length, e.rotate(-va * f.letterSpacing * (m - 1) / 2), s = 0; s < m; s += 1) q = j[s], 0 !== s && e.rotate(va * f.letterSpacing), e.save(), e.translate(0, -f.radius), f.flipArcText && e.scale(-1, -1), e.fillText(q, 0, 0), "transparent" !== f.fillStyle && (e.shadowColor = "transparent"), 0 !== f.strokeWidth && e.strokeText(q, 0, 0), e.restore();
            ...
            var g, i, j, k, l, m, q, t, u, v = this,
                w = Ea.imageCache;
            for (i = 0; i < v.length; i += 1) g = v[i], (j = h(v[i])) && (k = s(v[i]), l = new c(d), m = H(v[i], l, d, b), l.visible && (u = l.source, t = u.getContext, u.src || t ? q = u : u && (w[u] && w[u].complete ? q = w[u] : (q = new la, u.match(/^data:/i) || (q.crossOrigin = l.crossOrigin), q.src = u, w[u] = q)), q && (q.complete || t ? f(g, j, k, l, m)() : (q.onload = f(g, j, k, l, m), q.src = q.src))));
            return v
        },
        a.fn.createPattern = function (b) {
            ...
            var b, d, e, f, g, i, j, k, l, m, n = this,
                o = [];
